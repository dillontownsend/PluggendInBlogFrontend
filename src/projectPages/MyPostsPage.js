...
const MyPostsPage = () => {
    ...
    useEffect(() => {
        document.title = 'My Posts | PluggedIn'
        getLoggedIn()
        getMyPosts()
    }, [])
    ...
    const getMyPosts = async () => {
        const response = await fetch(`${env.API_URL}/blogposts/myposts`, {
            headers: {
                "x-access-token": `${localStorage.getItem('token')}`
            }
        })
    ...
    return (
        <div className="h-full min-h-screen bg-gray-lines bg-top-right">
            <NavBar currentlyLoggedIn={loggedIn} activePage="my posts" />

            <div className="w-4/5 lg:w-3/4 mx-auto">
                <h2 className="font-poppins tracking-tight text-5xl lg:text-6xl font-black text-center text-dark-gray-color mt-12 mb-4">{`${localStorage.getItem('username')}`}'s Posts</h2>
